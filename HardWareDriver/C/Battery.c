...
// ------------------------------------------------------------------------------------------------------------
// Get the temperature value of the ADC sampling internal temperature sensor
// Return value 3-digit temperature value XXX * 0.1C
int Get_Temp(void)
{
	u16 temp_val = 0;
	u8 t;
...
{
	Battery.BatteryAD = GetBatteryAD(); // Battery voltage detection
	...
	if (FLY_ENABLE)
	{
		...
		// In the state of flight such as the motor is turned on,
		// the alarm will start when the over-discharge voltage (BAT_OVERDIS_VAL) is above 0.03v
		...
		//Over-discharge protection, Battery over - discharge protect
	...
	{
		if ((Battery.BatteryVal < BAT_ALARM_VAL)
				&& (Battery.BatteryVal > BAT_CHG_VAL))
		{
			// Below 3.7v and greater than the charge detection voltage BAT_CHG_VAL
			Battery.alarm = 1;
		}
		else
		{
	...
	if (Battery.BatteryVal < BAT_CHG_VAL)
	{
		//on charge
		Battery.chargeSta = 1;
		BT_off();
