...
int16_t Motor[4] = { 0 };   //PWM motor defined array, respectively M1-M4
float rollSp = 0, pitchSp = 0;		//Re-calculating a desired roll pitch obtained according to the power distribution
float Thro = 0, Roll = 0, Pitch = 0, Yaw = 0;

//Examples of the structure of the PID ---- ----
PID_Typedef pitch_angle_PID;	//PID pitch angle of ring
PID_Typedef pitch_rate_PID;		//PID pitch angular rate of the ring

PID_Typedef roll_angle_PID;   //PID roll angle ring
PID_Typedef roll_rate_PID;    //PID roll angular velocity of the ring

PID_Typedef yaw_angle_PID;    //PID yaw angle ring
PID_Typedef yaw_rate_PID;     //PID yaw angular velocity of the ring
...
S_FLOAT_XYZ DIF_ACC;		//The actual difference to expect acceleration
S_FLOAT_XYZ EXP_ANGLE;	//The desired angle
S_FLOAT_XYZ DIF_ANGLE;	//Difference between the actual and the desired angle

uint32_t ctrlPrd = 0;
uint8_t headFreeMode = 0;
float headHold = 0;

//Function name: PID_Postion_Cal ()
//Description: The positional PID
static void PID_Postion_Cal(PID_Typedef *PID, float target, float measure,
	...
	PID->Output = (PID->P * PID->Error) + (PID->I * PID->Integ)
			+ (PID->D * PID->Deriv);    //PID: proportional + integral link link link + differential

	PID->PreError = PID->Error;

	//only for angular and angular velocity rings

	if (FLY_ENABLE && offLandFlag) {
		if (fabs(PID->Output) < Thro)		              //Larger than the throttle without integration
				{
			termI = (PID->Integ) + (PID->Error) * dt;     //Integral part
			if (termI > -PID->iLimit && termI < PID->iLimit
					&& PID->Output > -PID->iLimit && PID->Output < PID->iLimit) //Integrating links only when -300 to 300
				PID->Integ = termI;
		}
	} else {
		PID->Integ = 0;
...
//Function name: CtrlAttiAng (void)
//Description: control of the vehicle attitude (pitch, roll, yaw) control, Cascade PID loop control of the angle
void CtrlAttiAng(void) {
	...
	if (headFreeMode) {
		...
		float tarPitFree = angTarget[PITCH] * cosDiff
				+ angTarget[ROLL] * sinDiff;
		angTarget[ROLL] = angTarget[ROLL] * cosDiff
				- angTarget[PITCH] * sinDiff;
...
//Function name: CtrlAttiRate (void)
//Description: control of the vehicle attitude (pitch, roll, yaw) control, the angular velocity in Cascade PID loop control
void CtrlAttiRate(void) {
	...
	//Note that the original pid parameter value corresponding to the ad, so the switch
#ifdef IMU_SW
	PID_Postion_Cal(&pitch_rate_PID, pitch_angle_PID.Output,
			imu.gyro[PITCH] * 180.0f / M_PI_F, dt);
	PID_Postion_Cal(&roll_rate_PID, roll_angle_PID.Output,
			imu.gyro[ROLL] * 180.0f / M_PI_F, dt);       //gyroxGloble
	PID_Postion_Cal(&yaw_rate_PID, yawRateTarget,
			imu.gyro[YAW] * 180.0f / M_PI_F, dt);       //DMP_DATA.GYROz
#else
	
	//Parameter corresponds to the original direct output Gyro DMP is deg., And after the processing operation is the original error DMP
...
float holdAlt = 0;
uint8_t satZ = 0, satXY = 0;	//Whether supersaturated

#define ALT_LIMIT							2.0f		//Limit high 3.5
...
void CtrlAlti(void) {
	...
	thrustXYSpLen = sqrtf(
			thrustXYSp[0] * thrustXYSp[0] + thrustXYSp[1] * thrustXYSp[1]);
	//limit tilt max
	if (thrustXYSpLen > 0.01f) {
		...
		if (thrustXYSpLen > thrustXYMax) {
			...
			satXY = 1;
			thrustXYSpLen = sqrtf(
					thrustXYSp[0] * thrustXYSp[0]
							+ thrustXYSp[1] * thrustXYSp[1]);
		}
